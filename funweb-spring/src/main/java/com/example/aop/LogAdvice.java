...
- Proxy(프록시) : 프록시는 내부적으로 타깃을 포함해서 호출하는 객체. 자동으로 만들어짐(Auto Proxy 방식).
- JoinPoint(조인포인트) : 타깃 객체가 가진 여러 메소드 (후보 메소드들)
- PointCut(포인트컷) : 주요로직과 보조로직이 결합되는 지점. 조인포인트 중에서 포인트컷 표현식으로 결정된 메소드.
- Advice(어드바이스) : 보조로직을 가진 클래스. Aspect를 구현한 코드.
- Aspect(애스펙트) : 주변부 관심사(보조로직)을 의미하는 추상명사. 
*/

@Aspect // 주변부 관심사 로직을 가진 클래스
@Component
@Slf4j
public class LogAdvice {
	...
	public void logAfterReturning() {
		log.info("================ logAfterReturning() 호출됨 ================");
	}
	...
	public void logAfterThrowing(Exception exception) {
		log.info("================ logAfterThrowing() 호출됨 ================");
		log.info("Exception : {}", exception.getMessage());
	}
	
	@After("execution(* do*(*, *) )")
	public void logAfter() {
		log.info("================ logAfter() 호출됨 ================");
	}
	...
	public Object logTime(ProceedingJoinPoint pjp) throws Throwable {
		...
		// proceed() 메소드로 타깃 객체의 메소드 호출함
		// 호출 결과로 타깃 메소드의 리턴값을 반환함
		// 타깃 메소드의 리턴타입이 void 일 경우 null을 반환함
		Object result = pjp.proceed();
		log.info("result : {}", result);
		
		long endTime = System.currentTimeMillis();
		log.info("메소드 {} 실행시간: {}", signature.toShortString(), (endTime - startTime) + "ms");
